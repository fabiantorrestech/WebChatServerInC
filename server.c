...
request_type parse_request(recv_buffer *ps) {
  ...
  if(strstr(ps->requestbuf, "GET /listen") == ps->requestbuf)
    return sse_listen;
  // root should be used when you've received a complete request, and you know it's for index.html.
  else if(strstr(ps->requestbuf, "GET /") == ps->requestbuf)
    return root;
  ...
  // - "\r\n\r\n" is the terminating character for an HTTP header
  else if(strstr(ps->requestbuf, "\r\n\r\n") == NULL)
    return incomplete;
  // additional-case: NOT OKAY
  else
    return -1;
  ...
  for (i = 0; (i <= p->maxi) && (p->nready > 0); i++) {
    ...
    if ((connfd > 0) && (FD_ISSET(connfd, &p->ready_set))) {
      ...
      if ((n = read(connfd, &ps->requestbuf[offset], 8192 - offset)) != 0) {
        ...
        int result = 0;
        result = parse_request(ps);

        // ---- all cases of the requests ----
        if(result == root){ // ~~~~ 1
          // - 2 write n's
          //    + look at top chars *
        ...
        else if(result == speak){ // ~~~ 2
          // - Message (get msg with one line)
          // - send message to all clients.
          // - loop through pools of fds
          //    + check if fd in pool_of_fds is listening..
          //    + if yes, then you need to do some write-n's 
          writen(connfd, OK, strlen(OK));
          extract_message(ps->requestbuf, buf);
          close_and_remove(p, i);

          for(int j=0; j < sizeof(p->clientfd); j++){
            writen(p->clientfd[j], buf, strlen(buf));
          }

        }

        else if(result == sse_listen){ // ~~~ 3
          // listen
          // - flag that is set to tell whether listening or not? (structs) (in comments at top)
          //    - 1 = listening
          p->receiving_events[connfd] = 1;  
          writen(connfd, stream, strlen(stream));
        }
        
        else if(result == incomplete) // ~~~ 0
          Wait(NULL);
        
        else if(result == -1){ // ~~~ -1 (invalid request)
